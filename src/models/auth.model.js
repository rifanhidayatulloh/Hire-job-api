...
const authModel = {
  ...
  registerData: (data) => {
    return new Promise((resolve, reject) => {
      ...
      db.query(
        `INSERT INTO users(id, email, phone, password, level, is_active, is_verified, verify_token, photo, fullname, company, position)
        VALUES('${id}', '${email}', '${phone}', '${passwordHashed}', ${getLevel}, ${isActive}, ${isVerified}, '${verifyToken}', '${photo}', '${name}', '${company}', '${position}')`,
        (err, result) => {
          if (err) {
            // console.log(err);
            reject(err);
